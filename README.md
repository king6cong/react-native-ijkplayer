# react-native-ijkplayer [![npm version](https://badge.fury.io/js/react-native-ijkplayer.svg)](http://badge.fury.io/js/react-native-ijkplayer) [![Gitter](https://badges.gitter.im/lwansbrough/react-native-ijkplayer.svg)](https://gitter.im/lwansbrough/react-native-ijkplayer)
...
**NOTE** These docs are for the work in progress v1 release. If you want to use the latest and greatest and can deal with *significant* instability you can install with `npm install --save lwansbrough/react-native-ijkplayer`. If you are using older version of this module please refer to the [old readme](https://github.me.yiii/react-native-ijkplayer/tree/8cc61edef2c018b81e1c52f13c7d261fe6a35a63).
...
2. `npm install react-native-ijkplayer@https://github.me.yiii/react-native-ijkplayer.git --save`
3. `rnpm link react-native-ijkplayer`

### Mostly automatic install with CocoaPods
1. `npm install react-native-ijkplayer@https://github.me.yiii/react-native-ijkplayer.git --save`
2. Add the plugin dependency to your Podfile, pointing at the path where NPM installed it:
```
pod 'react-native-ijkplayer', path: '../node_modules/react-native-ijkplayer'
```
3. Run `pod install`
...
3. Go to `node_modules` ➜ `react-native-ijkplayer` and add `RCTIJKPlayer.xcodeproj`
4. In XCode, in the project navigator, select your project. Add `libRCTIJKPlayer.a` to your project's `Build Phases` ➜ `Link Binary With Libraries`
5. Click `RCTIJKPlayer.xcodeproj` in the project navigator and go the `Build Settings` tab. Make sure 'All' is toggled on (instead of 'Basic'). In the `Search Paths` section, look for `Header Search Paths` and make sure it contains both `$(SRCROOT)/../../react-native/React` and `$(SRCROOT)/../../../React` - mark both as `recursive`.
...
2. Open up `android/app/src/main/java/[...]/MainActivity.java
  - Add `import me.yiii.RCTIJKPlayer.*;` to the imports at the top of the file
  - Add `new RCTIJKPlayerPackage()` to the list returned by the `getPackages()` method
...
All you need is to `require` the `react-native-ijkplayer` module and then use the
`<Camera/>` tag.
...
import Camera from 'react-native-ijkplayer';
...
To see more of the `react-native-ijkplayer` in action, you can check out the `Example` folder.
